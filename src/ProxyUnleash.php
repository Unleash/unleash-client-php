...
class ProxyVariant
{
    public string $name;
    public bool $enabled;
    public ?ProxyVariantPayload $payload;

    public function __construct(string $name, bool $enabled, ?ProxyVariantPayload $payload)
    {
        $this->name = $name;
...
final class ProxyUnleash
{
    ...
    public function getVariant(string $featureName, ?Context $context = null, ?ProxyVariant $fallbackVariant = null): ProxyVariant
    {
        ...
        if (isset($body['variant']['payload']['type']) && isset($body['variant']['payload']['value'])) {
            $payload = new ProxyVariantPayload($body['variant']['payload']['type'], $body['variant']['payload']['value']);
        }

        if (isset($body['variant']) && isset($body['variant']['name']) && isset($body['variant']['enabled'])) {
            return new ProxyVariant($body['variant']['name'], $body['variant']['enabled'], $payload);
        } else {
            return $fallbackVariant ?? new ProxyVariant('disabled', false, null);
        }
    }
